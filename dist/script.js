...
const forms = () => {
  ...
  form.forEach(item => {
    item.addEventListener('submit', e => {
      ...
      let api;
      item.closest('.popup-design') || item.classList.contains('calc_form [data-calc="end"]') ? api = path.designer : api = path.question;

      for (let key in _calc__WEBPACK_IMPORTED_MODULE_2__["formState"]) {
        formData.append(key, _calc__WEBPACK_IMPORTED_MODULE_2__["formState"][key]);
      }
  ...
  links.forEach(link => {
    link.addEventListener('click', function (event) {
      ...
      let widthTop = document.documentElement.scrollTop,
          // расстояние пролистано сверху
      hash = this.hash,
          toBlock = document.querySelector(hash).getBoundingClientRect().top,
          // верхняя граница эл-та куда идет скролл
      start = null;
...
const showMoreStyles = (trigger, wrapper) => {
  ...
  btn.addEventListener('click', () => {
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_0__["getResource"])('assets/db.json').then(res => createCards(res.styles)).catch(() => {
      const errorMessage = document.createElement('div');
      errorMessage.textContent = 'Что-то пошло не так...';
      errorMessage.classList.add('status');
      errorMessage.style.cssText = 'text-align: center;font-size: 250%;color: red';
